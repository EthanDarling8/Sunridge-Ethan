...
function loadList() {
    dataTable = $('#DT_load').DataTable({
        ...
        "columns": [
            ...
            { data: "start", width: "10%" },
            { data: "end", width: "10%" },
            { data: "allDay", width: "10%" },
            { data: "id", width: "25%",
                "render": function (data) {
                    return `
                    <div class="text-center">
                        <a href="/Admin/Event/Upsert?eventId=${data}"
                            class="btn btn-warning text-white" style="cursor:pointer; width:100px;">
                                <i class="far fa-edit"></i> Edit
                        </a>

                        <a onClick=Delete('/api/event/'+${data})
...
function Delete(url) {
    swal({
        ...
        text: "This will also delete all associated data! You will not be able to restore the data!",
        icon: "warning",
        buttons: true,
        dangerMode: true
    }).then((willDelete) => {
